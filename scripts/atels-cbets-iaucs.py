...
for i in tqdm(range(len(yearseps) - 1)):
    ...
    atels.update(OrderedDict(sorted(
        [(list(filter(None, x.split('ATel.')[-1].split('.')))[0], x.strip()) for x in newatels])))
...
for i in tqdm(range(len(yearseps) - 1)):
    ...
    cbets.update(OrderedDict(sorted(
        [(list(filter(None, x.split('CBET.')[-1].split('.')))[0], x.strip()) for x in newcbets])))
...
for i in tqdm(range(len(yearseps) - 1)):
    ...
    iaucs.update(OrderedDict(sorted(
        [(list(filter(None, x.split('IAUC.')[-1].split('.')))[0], x.strip()) for x in newiaucs])))
