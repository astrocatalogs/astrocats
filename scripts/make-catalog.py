...
for fcnt, eventfile in enumerate(sorted(files, key=lambda s: s.lower())):
    ...
    if dohtml and args.writehtml:
        ...
        html = html.replace('<body>',
            '''<body class='event-body'><div style="padding-bottom:8px;"><strong>Disclaimer:</strong> All data collected by the OSC was originally generated by others, if you intended to use this data in a publication, we ask that you please cite the referenced sources and/or contact the sources of the data directly. Data sources can be determined by hovering over the data with your cursor.</div>''')
