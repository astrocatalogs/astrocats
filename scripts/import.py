...
def add_event(name, load = True, delete = True, source = ''):
...
if 'writeevents' in tasks:
    if args.update:
        ...
        for fi in files:
            ...
            events[name] = OrderedDict(([['name', events[name]['name']]] + ([['alias', events[k]['alias']]] if 'alias' in events[k] else []) + [['stub', True]]))
    else:
