...
def do_vizier(catalog):
    ...
    for row in pbar(table, current_task):
        ...
        catalog.events[name].add_quantity('ebv', str(
    ...
    for row in pbar(table, current_task):
        ...
        catalog.events[name].add_quantity('ebv', str(row['E_B-V_']), source)
        catalog.events[name].add_quantity('ra', row['RAJ2000'], source)
        catalog.events[name].add_quantity('dec', row['DEJ2000'], source)
    ...
    for row in pbar(table, current_task):
        ...
        catalog.events[name].add_quantity('alias', name, source)
        add_photometry(
            catalog.events, name, time=str(row['MJD']), band=row['Filter'],
            instrument=row['Inst'],
    ...
    for row in pbar(table, current_task):
        ...
        if band == 'Z':
            telescope = 'Subaru'
        add_photometry(
            catalog.events, name, time=str(row['MJD']), band=band, system=system,
            telescope=telescope,
            magnitude=magnitude, e_magnitude=e_magnitude, source=source)
    ...
    for row in pbar(table, current_task):
        ...
        catalog.events[name].add_quantity('discoverdate', make_date_string(
        ...
        catalog.events[name].add_quantity('claimedtype', type_str, source)
        catalog.events[name].add_quantity('ra', row['RAJ2000'], source)
        catalog.events[name].add_quantity('dec', row['DEJ2000'], source)
    catalog.journal_events()
    ...
    for row in pbar(table, current_task):
        ...
        if row['mag'] != '--':
            add_photometry(
                catalog.events, name, time=str(row['MJD']), band=row['Filt'],
                magnitude=str(row['mag']),
    ...
    for row in pbar(table, current_task):
        ...
        catalog.events[name].add_quantity('alias', row['SNR'].strip(), source)
        catalog.events[name].add_quantity(
        ...
        if row['Names']:
            names = row['Names'].split(',')
            for nam in names:
                catalog.events[name].add_quantity('alias', nam.replace(
                    'Vela (XYZ)', 'Vela').strip('()').strip(), source)
                if nam.strip()[:2] == 'SN':
                    catalog.events[name].add_quantity(
                        'discoverdate', nam.strip()[2:], source)

        catalog.events[name].add_quantity('host', 'Milky Way', source)
        catalog.events[name].add_quantity('ra', row['RAJ2000'], source)
        catalog.events[name].add_quantity('dec', row['DEJ2000'], source)
    catalog.journal_events()
    ...
    for row in pbar(table, current_task):
        ...
        catalog.events[name].add_quantity('alias', name, source)
        add_photometry(
            catalog.events, name, time=str(jd_to_mjd(Decimal(row['JD']))),
            instrument='QUEST',
    ...
    for row in pbar(table, current_task):
        ...
        catalog.events[name].add_quantity('alias', name, source)
        add_photometry(
            catalog.events, name, time=row['MJD'], band=row[
    ...
    for row in pbar(table, current_task):
        ...
        for band in ['B', 'R']:
            ...
            if (bandtag in row and is_number(row[bandtag]) and not
                    isnan(float(row[bandtag]))):
                add_photometry(catalog.events, name, time=row["MJD"], band=band,
                               magnitude=row[bandtag],
    ...
    for row in pbar(table, current_task):
        ...
        for band in ['J', 'H', 'Ks']:
            ...
            if (bandtag in row and is_number(row[bandtag]) and not
                    isnan(float(row[bandtag]))):
                add_photometry(catalog.events, name, time=row["MJD"],
    ...
    for row in pbar(table, current_task):
        ...
        for band in ['U', 'B', 'V', 'R', 'I']:
            ...
            if (bandtag in row and is_number(row[bandtag]) and not
                    isnan(float(row[bandtag]))):
                add_photometry(catalog.events, name, time=mjd, telescope=row["Tel"],
    ...
    for row in pbar(table, current_task):
        ...
        for band in ['B', 'V', 'R', 'I']:
            ...
            if (bandtag in row and is_number(row[bandtag]) and not
                    isnan(float(row[bandtag]))):
                add_photometry(catalog.events, name, time=mjd, telescope="LJT",
    ...
    for row in pbar(table, current_task):
        ...
        for band in ['U', 'B', 'V', 'UVW1', 'UVW2', 'UVM2']:
            ...
            if (bandtag in row and is_number(row[bandtag]) and not
                    isnan(float(row[bandtag]))):
                add_photometry(catalog.events, name, time=mjd, telescope="Swift",
    ...
    for row in pbar(table, current_task):
        ...
        catalog.events[name].add_quantity('alias', name, source)
        add_photometry(catalog.events, name, time=str(jd_to_mjd(Decimal(row['JD']))),
    ...
    for row in pbar(table, current_task):
        ...
        catalog.events[name].add_quantity(
            ...
            error=row['e_zSN'])
        catalog.events[name].add_quantity('hostra', row['RAG'], source)
        catalog.events[name].add_quantity('hostdec', row['DEG'], source)
        catalog.events[name].add_quantity(
    ...
    for row in pbar(table, current_task):
        ...
        catalog.events[name].add_quantity('maxdate', make_date_string(
            astrot.year, astrot.month, astrot.day), source)
    catalog.journal_events()
    ...
    for row in pbar(table, current_task):
        ...
        for band in ['UVW2', 'UVM2', 'UVW1', 'U', 'B', 'V']:
            ...
            if (bandtag in row and is_number(row[bandtag]) and not
                    isnan(float(row[bandtag]))):
                add_photometry(catalog.events, name, time=str(row["MJD"]), band=band,
                               magnitude=row[bandtag],
