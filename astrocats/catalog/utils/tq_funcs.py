...
def tq(li, currenttask='', leave=True):
    return tqdm(list(li), desc=currenttask, leave=leave, dynamic_ncols=True)


def pbar(iter, desc='', **kwargs):
    """Wrapper for `tqdm` progress bar.
    """
    return tqdm(iter, desc=desc, dynamic_ncols=True, **kwargs)


def pbar_strings(files, desc='', **kwargs):
    """Wrapper for `tqdm` progress bar which also sorts list of strings
    """
    return tqdm(sorted(files, key=lambda s: s.lower()), desc=desc,
                dynamic_ncols=True, **kwargs)
